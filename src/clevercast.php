<?php
# PHP Client Library for Clevercast
# Copyright (C) 2017 rambla.eu
#
...
class Clevercast
{
   ...
   * Constructor.
   *
   * @param string $username Username for Basic Authentication to Clevercast
   * @param string $pwd Password for Basic Authentication to Clevercast
   ...
   * @param string $custom_host To use a custom Clevercast host name 
   ...
   */
    ...
    $this->user_agent = "Clevercast 1.0";
    if ($user_agent) {
  ...
  /**
   * Check if error response was received from Clevercast.
   *
   * @param Requests_Response $response HTTP Response class
   * @throws Exception If an error response was returned by Clevercast
   */
  ...
  /**
   * Send a PUT items request to clevercast to update the snapshot that is being used for a given item.
   *
   * @param string $item_id ID of the item.
   ...
   */
	...
	/**
   * POST a file to Clevercast's items-hmac endpoint.
   *
   ...
   * @param string $account_id ID of your Clevercast account.
   * @param string $wprofile_id ID of your Clevercast workflow profile.
   * @param string $auth_secret Shared secret attached to your workflow profile.
   * @param int $hmac_valid_seconds Number of seconds during which the HMAC will be considered as valid by Clevercast (default = 30).
   ...
   * @return JSON object containing the URL(s) required to retrieve the item's data through the Clevercast items API, or containing an error
   */
